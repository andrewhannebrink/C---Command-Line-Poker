...
//This source file defines the class Razz declared in razz.h
...
int Razz::after_round() {
	...
	//Get the worst hand for each player (even though we'll use the bestHand_ member variable.)
	...
	sort(pvIn.begin(), pvIn.end(), lowstud_player_poker_rank);
	pvIn[0]->wins_++;
	...
	for (int i = 0; i <pvIn.size(); i++) {
		...
		std::cout<<pvIn[i]->playerName_<<"'s worst hand was: "<<pvIn[i]->bestHand_.asString()<<", ";
...
int Razz::before_turn(Player &) {
	return 0;
}
	
int Razz::turn(Player &) {
	return 0;
}